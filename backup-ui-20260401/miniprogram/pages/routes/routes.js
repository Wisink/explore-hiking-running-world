    ...
    if (filter !== 'all') {
      result = data.filter(item => {
        switch (filter) {
          ...
          case 'stone':
            return (item.sections || []).some(s => s.road && s.road.includes('石'))
          case 'season':
            const curSeason = getSeasonTag().label.replace('去', '') // '春天' '夏天' '秋天' '冬天'
            const bestSeason = item.bestSeason || item.best_season || []
            const seasonStr = Array.isArray(bestSeason) ? bestSeason.join(',') : String(bestSeason)
            return seasonStr.includes(curSeason) || (curSeason === '秋天' && (item.scenery || []).some(f => f.includes('红叶') || f.includes('银杏') || f.includes('金黄')))
          case 'free':
            return item.isFree
          case 'east':
            return item.location_direction && item.location_direction.includes('东线')
          case 'center':
            return item.location_direction && item.location_direction.includes('中线')
          case 'west':
            return item.location_direction && item.location_direction.includes('西线')
          default:
            return true
        }
      })
