...
function parseRouteInfo(searchResults, routeName) {
  ...
  if (/困难|难度大|危险|慎入|偏难|技术/.test(allText)) {
    ...
    info.difficulty = 3
  } else if (/轻松|简单|新手|亲子|休闲|平缓/.test(allText)) {
    info.difficulty = 2
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (action === 'setApiKey') {
      ...
      await db.collection('config').doc('tavily_search').set({
        data: { apiKey, updatedAt: db.serverDate() }
      })
      return success(null, 'Tavily API Key已保存')
    }
    ...
    if (action === 'searchAndParse') {
      ...
      let allResults = []
      try {
        const res = await tavilySearch(`${name} 徒步攻略 路线 距离 海拔`, 10)
